...
class DetectorBase(metaclass=abc.ABCMeta):
    ...
    def setAdressCamera(self, video_path=0, frame_step=1):
        ...
        if frame_step == 1:
            self.frame_streamer = streams.g_from_cv_capture(video_path)
        elif frame_step < 1:
            raise ValueError('Шаг кадров не может быть меньше 1!')
        else:
            video_stream = streams.g_from_cv_capture(video_path)
            self.frame_streamer = streams.g_thin_out_frames(video_stream, frame_step)
    ...
    def streamFromOneFile(self, video_path):
        self.video_path = video_path
        self.frame_streamer = streams.g_from_one_file( video_path )


    def streamFromDirectory(self, video_path):
        self.video_path = video_path
        self.frame_streamer = streams.g_from_directory( video_path )


    def frame_stream(self):
        for rgb, metadata in self.frame_streamer:
    ...
    def refitKNN(self):
        if self.distance_search_type == 'KNN':
            ...
            self.knn = neighbors.NearestNeighbors(n_neighbors=1, algorithm='auto', metric=self.distance_metric).fit( list(self.dataframe['embedding'].array) )
        else:
            self.knn = None
